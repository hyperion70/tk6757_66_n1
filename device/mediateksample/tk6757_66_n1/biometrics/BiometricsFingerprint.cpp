...
typedef struct fingerprint_dev {
    ...
    const char *vendor;
} fingerprint_hal_module_t;
fingerprint_hal_module_t fingerprint_list[] = {
	{
        .module_id = "gxfingerprint",  //.so name
        .vendor = "goodix_fp"  //sensor name
    },
    {
...
fingerprint_device_t* BiometricsFingerprint::openHal() {
    ...
    int len = sizeof(fingerprint_list) / sizeof(fingerprint_hal_module_t);
    for(int i = 0; i < len; i++)
    {
            if (0 != (err = hw_get_module(fingerprint_list[i].module_id, &hw_mdl))) {
                ALOGE("Can't open fingerprint HW Module, id:%s, error: %d", fingerprint_list[i].module_id, err);
        return nullptr;
    }
            ALOGD("Open fingerprint HW Module:%s success!", fingerprint_list[i].module_id);
    ...
    if (0 != (err = module->common.methods->open(hw_mdl, nullptr, &device))) {
        ALOGE("Can't open fingerprint methods, error: %d", err);
                ...
                ALOGD("%s methods open success!", fingerprint_list[i].module_id);
                break;
            }
    } 
    if(device == nullptr){
        ALOGE("All fingerprint module open fail,Please check!");
        return nullptr;
    }
