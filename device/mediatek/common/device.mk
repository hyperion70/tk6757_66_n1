...
PRODUCT_COPY_FILES += $(MTK_TARGET_PROJECT_FOLDER)/ProjectConfig.mk.bak:$(TARGET_COPY_OUT_VENDOR)/data/misc/ProjectConfig.mk:mtk
...
# Add for ModemMonitor(MDM) framework
  ifeq ($(strip $(MTK_MODEM_MONITOR_SUPPORT)),yes)
    ...
    PRODUCT_PACKAGES += \
      ...
      MDMLSample \
      MDMConfig
    INTERNAL_ICDPATH := vendor/mediatek/proprietary/frameworks/opt/mdm/icdcodec/jni/include/AllICD.json
    CUSTOMER_ICDPATH := vendor/mediatek/proprietary/frameworks/opt/mdm_lib/libicdcodec_jni/icdcodec/jni/include/AllICD.json
    ifneq ($(wildcard $(INTERNAL_ICDPATH)),)
      PRODUCT_COPY_FILES += $(INTERNAL_ICDPATH):$(TARGET_COPY_OUT_SYSTEM)/etc/AllICD.json:mtk
    else
      PRODUCT_COPY_FILES += $(CUSTOMER_ICDPATH):$(TARGET_COPY_OUT_SYSTEM)/etc/AllICD.json:mtk
    endif
  endif
  ...
  ifeq ($(strip $(MTK_MDLOGGER_SUPPORT)),yes)
    ...
    ifneq ($(strip $(MTK_MD1_SUPPORT)),)
      ifneq ($(strip $(MTK_MD1_SUPPORT)),0)
        ...
        PRODUCT_PROPERTY_OVERRIDES += ro.vendor.md_log_memdump_wait=0
      endif
    endif
