...
/*----------------------------------------------------------------------------*/
static int LSM6DS3H_gyro_write_rel_calibration(struct lsm6ds3h_gyro_i2c_data *obj,
...
static int LSM6DS3H_gyro_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (hwmsen_read_byte(client, LSM6DS3H_CTRL2_G, databuf)) {
		GYRO_PR_ERR("read lsm6ds3h power ctl register err!\n");
		return LSM6DS3H_ERR_I2C;
	}
#endif
...
static int LSM6DS3H_Set_RegInc(struct i2c_client *client, bool inc)
{
	...
	if (res < 0) {
		GYRO_PR_ERR("1 read LSM6DS3H_CTRL1_XL err!\n");
		return LSM6DS3H_ERR_I2C;
	}
#else
	if (hwmsen_read_byte(client, LSM6DS3H_CTRL3_C, databuf)) {
		GYRO_PR_ERR("2 read LSM6DS3H_CTRL1_XL err!\n");
		return LSM6DS3H_ERR_I2C;
	}
#endif
	else
...
#endif
		if (res <= 0) {
			GYRO_PR_ERR("write full scale register err!\n");
			return LSM6DS3H_ERR_I2C;
		}
	}
	return LSM6DS3H_SUCCESS;
...
static int LSM6DS3H_gyro_SetFullScale(struct i2c_client *client, u8 gyro_fs)
{
	...
	if (res < 0) {
		GYRO_PR_ERR("read LSM6DS3H_CTRL2_G err!\n");
		return LSM6DS3H_ERR_I2C;
	}
#else
	if (hwmsen_read_byte(client, LSM6DS3H_CTRL2_G, databuf)) {
...
s32 LSM6DS3H_gyro_TransfromResolution(s32 rawData)
{
	...
	tempValue = (s64) (rawData) * LSM6DS3H_GYRO_SENSITIVITY_2000DPS * 3141592;
	if (tempValue < 0) {
...
static int LSM6DS3H_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (hwmsen_read_block(client, LSM6DS3H_OUTX_L_G, databuf, 6)) {
		GYRO_PR_ERR("LSM6DS3H read gyroscope data  error\n");
		return -2;
	}
...
#ifdef LSM6DS3H_SENSITIVITY_SCALING
	data[LSM6DS3H_AXIS_X] =
	    LSM6DS3H_gyro_scale(data[LSM6DS3H_AXIS_X], LSM6DS3H_SENSITIVITY_SCALING_X);
	data[LSM6DS3H_AXIS_Y] =
	    LSM6DS3H_gyro_scale(data[LSM6DS3H_AXIS_Y], LSM6DS3H_SENSITIVITY_SCALING_Y);
	data[LSM6DS3H_AXIS_Z] =
	    LSM6DS3H_gyro_scale(data[LSM6DS3H_AXIS_Z], LSM6DS3H_SENSITIVITY_SCALING_Z);
#endif
...
static int gyroscope_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	GYRO_LOG("lsm6ds3hg gyro set delay = (%d) ok.\n", value);
...
static int lsm6ds3h_gyro_get_data(int *x, int *y, int *z, int *status)
{
	...
	if (sscanf(buff, "%x %x %x", x, y, z) != 3)
		GYRO_LOG("get data format error\n");
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;

	return 0;
}

static int lsm6ds3hg_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err = 0;
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			if (sscanf(buff, "%x %x %x", &gyro_data->values[0],
			       &gyro_data->values[1], &gyro_data->values[2]) != 3)
				GYRO_LOG("get data format error\n");
...
static long lsm6ds3h_gyro_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	default:
		GYRO_PR_ERR("%s not supported = 0x%04x", __func__, cmd);
		return -ENOIOCTLCMD;
...
static long lsm6ds3h_gyro_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GYROSCOPE_IOCTL_READ_SENSORDATA:
		...
		if (LSM6DS3H_ReadGyroData(client, strbuf, LSM6DS3H_BUFSIZE) == 0) {
			int x, y, z;

			/*enlarged 100 times in kernel level, need to shrink it in factory case */
			if (sscanf(strbuf, "%x %x %x", &x, &y, &z) != 3)
				GYRO_PR_ERR("ioctl readGyro error!\n");
			sprintf(strbuf, "%x %x %x", x / 1000, y / 1000, z / 1000);
		}
	...
	case GYROSCOPE_IOCTL_SET_CALI:
		...
		if (copy_from_user(&sensor_data, data, sizeof(sensor_data))) {
			...
			cali[LSM6DS3H_AXIS_X] =
				(s64) (sensor_data.x);	/* * 180*1000*1000/(LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142);*/
			cali[LSM6DS3H_AXIS_Y] =
				(s64) (sensor_data.y);	/* * 180*1000*1000/(LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142);*/
			cali[LSM6DS3H_AXIS_Z] =
				(s64) (sensor_data.z);	/* * 180*1000*1000/(LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142);*/
	...
	case GYROSCOPE_IOCTL_GET_CALI:
		...
		sensor_data.x =
			(s64) (cali[LSM6DS3H_AXIS_X]);	/* * LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);*/
		sensor_data.y =
			(s64) (cali[LSM6DS3H_AXIS_Y]);	/* * LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);*/
		sensor_data.z =
			(s64) (cali[LSM6DS3H_AXIS_Z]);	/* * LSM6DS3H_GYRO_SENSITIVITY_2000DPS*3142/(180*1000*1000);*/
...
static int lsm6ds3h_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		GYRO_PR_ERR("lsm6ds3h_gyro_device misc register failed!\n");
	...
	if (err) {
		GYRO_PR_ERR("lsm6ds3h create attribute err = %d\n", err);
...
static int lsm6ds3h_gyro_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		GYRO_PR_ERR("lsm6ds3h_gyro_i2c_remove fail: %d\n", err);
