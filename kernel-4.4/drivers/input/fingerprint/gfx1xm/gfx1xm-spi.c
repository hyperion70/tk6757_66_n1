...
#include <linux/wakelock.h>               //Jorbo20171016
//#include "spi_dts_gpio.h"

#define WAKELOCK_HOLD_TIME 3000//500 /* in ms */  //Jorbo20171016
static struct wake_lock fp_wakelock; //Jorbo20171016
static DECLARE_BITMAP(minors, N_SPI_MINORS);
static DECLARE_WAIT_QUEUE_HEAD(waiter);
static int suspend_test_flag = 0;	
int factory_test_version=0;	//add for factory test, wangqiang, @2016-6-12 16:39:50
...
{
	g_debug_level=level;
	gfx1xm_debug(DEFAULT_DEBUG,"g_debug_level=%d",g_debug_level);
	return 0;
}
...
static void gf_enable_irq(struct gfx1xm_dev* gfx1xm_dev)
{
	if(gfx1xm_dev->irq_enabled_flag) {
		gfx1xm_debug(DEFAULT_DEBUG,"irq has been enabled.\n");
	} else {
		gfx1xm_debug(DEFAULT_DEBUG,"called.\n");
		enable_irq(gfx1xm_dev->spi->irq);
		gfx1xm_dev->irq_enabled_flag = 1;
	}
}
static void gf_disable_irq(struct gfx1xm_dev* gfx1xm_dev)
{
	if(gfx1xm_dev->irq_enabled_flag) {
		gfx1xm_dev->irq_enabled_flag = 0;
		gfx1xm_debug(DEFAULT_DEBUG,"called.\n");
		disable_irq(gfx1xm_dev->spi->irq);
	} else {
		gfx1xm_debug(DEFAULT_DEBUG,"irq has been disabled.\n");
	}
}
static void gfx1xm_spi_set_mode(struct spi_device *spi, SPI_SPEED speed, int flag)
{
	...
	if(spi_setup(spi) < 0){
		gfx1xm_error("Failed to set spi.");
	}
}
...
//		gfx1xm_error("%s:size not match [%d:%d]",__func__,GFX1XM_CFG_LEN, count);
		return -EFAULT;
	}

	mutex_lock(&gfx1xm_dev->buf_lock);
	if (copy_from_user(&gfx1xm_dev->buffer[GFX1XM_WDATA_OFFSET], buffer, count)){
		gfx1xm_error("%s:copy from user fail",__func__);
		mutex_unlock(&gfx1xm_dev->buf_lock);
		return -EFAULT;		
	}
				...
				u16 addr, u32 data_len, u8 *tx_buf)
{
	...
	if((package_num > 0) && (reminder != 0)) {
        ...
        gfx1xm_debug(SPI_DEBUG,"w-reminder:0x%x-0x%x,0x%x",reminder_buf[GFX1XM_WDATA_OFFSET],reminder_buf[GFX1XM_WDATA_OFFSET+1],
	...
	if(ret == 0) {
		...
		gfx1xm_debug(SPI_DEBUG,"write async failed. ret = %d",ret);
	}
				...
				u16 addr, u32 data_len, u8 *rx_buf)
{
	...
	if(ret == 0) {
		if(twice == 1) {
            gfx1xm_debug(SPI_DEBUG,"reminder:0x%x:0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x",reminder_buf[0], reminder_buf[1], 
        ...
        gfx1xm_error("read failed. ret = %d",ret);
  }
...
static int gfx1xm_factory_test(struct gfx1xm_dev *gfx1xm_dev)
{
	//int error;
	int status=0;

	
	printk("gfx1xm: factory_test_version = 0x%x\n", factory_test_version);

//modify begin by yangweisong 2017 02 23	
	if (factory_test_version == gfx1xm_dev->chip_version_config) { //chip_version_config
		//dev_err(&fpc1080->spi->dev, "hardware id matched: %x\n",fpc1080->huge_buffer[fpc1080->data_offset]);
		printk("vendor id matched:  expected %x\n",factory_test_version);
		
		status=1;//test success

	}
	else{
		//dev_err("hardware id mismatch: %x expected %x\n",fpc1080->huge_buffer[fpc1080->data_offset],	FPC1080_HARDWARE_ID);
		printk("vendor id mismatch:  expected %x\n",factory_test_version);
		status=0;//test fail
	}
//modify end by yangweisong 2017 02 23
...
//add by xiaolong 20160601 end	
	...
	case GFX1XM_IOC_CMD:
	    ...
	    if(ioc->cmd == GFX1XM_R) {
			...
			if(ioc->addr == GFX1XM_MODE_STATUS) {
				...
				gfx1xm_dev->old_mode = gfx1xm_dev->mode;
			...
			if((gfx1xm_dev->old_mode==GFX1XM_FF_MODE)&&(gfx1xm_dev->mode!=GFX1XM_FF_MODE)){
				mdelay(75);
				}
	    }
#else
	...
	case GFX1XM_IOC_REINIT:
		mutex_lock(&gfx1xm_dev->fb_lock);
	    gf_disable_irq(gfx1xm_dev);
	    gfx1xm_hw_reset(gfx1xm_dev, 60);
		gf_enable_irq(gfx1xm_dev);
	...
	case GFX1XM_IOC_SLEEP:
	    ...
	    break;
	//add by xiaolong 20160601 start
	case GFX1XM_IOC_FACTORY_TEST:
		status = gfx1xm_factory_test(gfx1xm_dev);
		printk(KERN_ERR "GFX1XM_IOC_FACTORY_TEST, status=%d!\r\n",status);
		retval = copy_to_user(ptr, &status, sizeof(status));
		break;
	//add by xiaolong 20160601 end  
	default:
...
static int isUpdate(struct gfx1xm_dev *gfx1xm_dev)
{
    ...
    gfx1xm_debug(DEFAULT_DEBUG,"0x41e4 = 0x%x",fw_running);
    if(fw_running == 0xbe) {
		/*firmware running*/		
		if(gfx1xm_dev->chip_version_config == 0x38){
			ver_file = (int)(fw318[12] & 0xF0) <<12;
			ver_file |= (int)(fw318[12] & 0x0F)<<8;
			ver_file |= fw318[13];	//get the fw version in the i file;
		}else if(gfx1xm_dev->chip_version_config == 0x36){
...
{	
	...
	if (retry >= 3){
		gfx1xm_debug(DEFAULT_DEBUG, "ESD check has a problem");
		gfx1xm_hw_reset(gfx1xm_dev, 60);
...
{    
	...
	gfx1xm_dev->early_suspend_flag = 1;
#if ESD_PROTECT		
...
#endif
	gfx1xm_dev->early_suspend_flag = 0;
}
#endif
/*******************************************
**Interrupter
...
static irqreturn_t gfx1xm_irq(int irq, void* dev)
{
	...
	struct gfx1xm_dev *gfx1xm_dev = dev;
	wake_lock_timeout(&fp_wakelock, msecs_to_jiffies(WAKELOCK_HOLD_TIME));//Jorbo20171016
	gfx1xm_dev->irq_flag = 1;
    //irq_flag = 1;
    gfx1xm_debug(DEFAULT_DEBUG, "entry");
	//gfx1xm_eint_func();
	//disable_irq_nosync(gfx1xm_dev->spi->irq);//20160307
...
static int gfx1xm_event_handler(void *para)
{
	...
	do{
		...
		wait_event_interruptible(waiter, gfx1xm_dev->irq_flag != 0);
		mutex_lock(&gfx1xm_dev->fb_lock);
		gfx1xm_dev->irq_flag = 0;

		if(suspend_test_flag == 1)
		{
			gfx1xm_debug(DEFAULT_DEBUG,"suspend flag is true");
			mutex_unlock(&gfx1xm_dev->fb_lock);
			continue;
		}
		...
		if (status_1 & GFX1XM_BUF_STA_MASK){
			gfx1xm_debug(DEFAULT_DEBUG, "ESD reset irq,status_1=0x%02X",status_1);
			gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS+1, status_1&0x7F);
			if (1 == gfx1xm_dev->early_suspend_flag){
	    ...
	    if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT == get_boot_mode())
	    {
	        if(mode != GFX1XM_KEY_MODE)
	        {
	        	...
	        	gfx1xm_debug(DEFAULT_DEBUG,"factory to set key mode.");
				mutex_unlock(&gfx1xm_dev->fb_lock);
	        	continue;
	        }
		 	gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, 0x00); //clean buf
		...
		case GFX1XM_FF_MODE://FF mode could get data
		    if((status & GFX1XM_HOME_KEY_MASK) && (status & GFX1XM_HOME_KEY_STA)){
				gfx1xm_debug(DEFAULT_DEBUG,"wake device.");
		...
		case GFX1XM_KEY_MODE:
		    gfx1xm_debug(DEFAULT_DEBUG,"Key mode: status = 0x%x",status);
		...
		case GFX1XM_SLEEP_MODE:
		    gfx1xm_error("Should not happen in sleep mode.");
		    break;
		case GFX1XM_DEBUG_MODE:
		    ...
		    break;
		default:
		    gfx1xm_error("Unknown mode. mode = 0x%x",mode);
		    break;
	    }
		mutex_unlock(&gfx1xm_dev->fb_lock);
...
static int gfx1xm_open(struct inode *inode, struct file *filp)
{
    ...
    if(status == 0){
		...
		if(status == 0) {
		    ...
		    enable_irq_wake(gfx1xm_dev->spi->irq);
		    gf_enable_irq(gfx1xm_dev);
		}
	} else {
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	while(rery--){
		...
		if (0xAA == tmp){
			gfx1xm_debug(DEFAULT_DEBUG,"read Vendor ID\n");
			break;
		}
		gfx1xm_debug(DEFAULT_DEBUG,"read 0x8042=0x%02X retry=%d\n", tmp, rery);
		msleep(5);
	}
	...
	while(rery--){
		...
		if (0 == tmp){
			gfx1xm_debug(DEFAULT_DEBUG,"read Vendor ID is ready!!!\n");
			break;
		}
		gfx1xm_debug(DEFAULT_DEBUG,"read 0x5094=0x%02X retry=%d\n", tmp, rery);
		msleep(5);
	}
	//read Vendor ID
	...
	if(ret <= 0)
		gfx1xm_error("write config fail");
	mdelay(200);
	return 1;
}
#endif
...
static int gfx1xm_check_9p_chip(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	do{
		...
		gfx1xm_debug(DEFAULT_DEBUG,"9p chip version is 0x%02X, 0x%02X, 0x%02X, 0x%02X\n", reg_value[GFX1XM_RDATA_OFFSET], reg_value[GFX1XM_RDATA_OFFSET+1], reg_value[GFX1XM_RDATA_OFFSET+2], reg_value[GFX1XM_RDATA_OFFSET+3]);
		time_out++;
		if ( (0x00 == reg_value[GFX1XM_RDATA_OFFSET+3])&& (0x90 == reg_value[GFX1XM_RDATA_OFFSET+2]) && (0x08 == reg_value[GFX1XM_RDATA_OFFSET+1])){
			gfx1xm_debug(DEFAULT_DEBUG,"9p chip version check pass!, time_out=%d\n", time_out);
			return 1;
		}
	}while(time_out<30);
	
	gfx1xm_debug(DEFAULT_DEBUG,"9p chip version check failed!, time_out=%d\n",time_out);
	return 0;
}
...
static int  gfx1xm_probe(struct spi_device *spi)
{
	...
	/* Initialize and Allocate the driver data */
	gfx1xm_dev->spi = spi;
	...
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,goodix_fp");
	gfx1xm_dev->pctrl = devm_pinctrl_get(&spi->dev);
	if (IS_ERR(gfx1xm_dev->pctrl)) {
		ret = PTR_ERR(gfx1xm_dev->pctrl);
		dev_err(&spi->dev, "gfx1xm Cannot find fp pctrl!\n");
		return ret;
	}
	//flags param for goodix init
	gfx1xm_dev->irq_flag = 0;
	gfx1xm_dev->old_mode = 0;
	gfx1xm_dev->early_suspend_flag = 0;
	gfx1xm_dev->chip_version_config = 0;
	gfx1xm_dev->irq_enabled_flag = 1;
	...
	if (!gfx1xm_check_9p_chip(gfx1xm_dev)){
		gfx1xm_error("9p chip version check failed!");
	}
	...
	factory_test_version = version[4];//add for factory test, wangqiang, @2016-6-12 16:39:50
#if defined GFX18M_PID
		gfx1xm_dev->chip_version_config = 0x38;
#elif defined GFX16M_PID
		gfx1xm_dev->chip_version_config = 0x36;
#else
		#error "have't define any product for fw"
#endif

	gfx1xm_debug(DEFAULT_DEBUG,"chip_version_config=%x ",gfx1xm_dev->chip_version_config);
    ...
    gfx1xm_dev->gfx1xm_spi_class = class_create(THIS_MODULE, CLASS_NAME);
    if (IS_ERR(gfx1xm_dev->gfx1xm_spi_class)) {
		gfx1xm_error("Failed to create class.");
		FUNC_EXIT();
		status = PTR_ERR(gfx1xm_dev->gfx1xm_spi_class);
	...
	if (minor < N_SPI_MINORS) {
		...
		dev = device_create(gfx1xm_dev->gfx1xm_spi_class, &spi->dev, gfx1xm_dev->devt,
			  gfx1xm_dev, DEV_NAME);
	...
	gfx1xm_dev->gfx1xm_irq_thread = kthread_run(gfx1xm_event_handler, (void *)gfx1xm_dev, "gfx1xm");
	if (IS_ERR(gfx1xm_dev->gfx1xm_irq_thread)) {
		gfx1xm_error("Failed to create kernel thread: %ld",PTR_ERR(gfx1xm_dev->gfx1xm_irq_thread));
	}
	...
	if(finger_irq_node) {
		...
		if(!gfx1xm_dev->spi->irq){
			gfx1xm_debug(DEFAULT_DEBUG,"irq_of_parse_and_map fail!!\n");
			goto err_free_input;		
		}
		wake_lock_init(&fp_wakelock, WAKE_LOCK_SUSPEND, "fp_wakelock");//Jorbo 20171016
		if(request_irq(gfx1xm_dev->spi->irq,gfx1xm_irq,IRQF_TRIGGER_RISING,"hall_2-eint",gfx1xm_dev)){
			gfx1xm_debug(DEFAULT_DEBUG,"IRQ LINE NOT AVAILABLE!!\n");
			enable_irq_wake(gfx1xm_dev->spi->irq);//add by chenxing
//			goto err_free_input;		
		}
#if FACTORY_MODE_TEST
		if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT == get_boot_mode()) {
			gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_MODE_STATUS, 0x01); //write to key mode
			gfx1xm_debug(DEFAULT_DEBUG,"probe set key mode.");
		}
		else		
#endif
		{
		...
		gfx1xm_debug(DEFAULT_DEBUG,"finger_irq_node not available!!\n");
		goto err_free_input;
	}
	...
	gfx1xm_dev->mode = GFX1XM_IMAGE_MODE;
	FUNC_EXIT();
...
err_alloc_input:
err_creat_group:
	class_destroy(gfx1xm_dev->gfx1xm_spi_class);
err_creat_class:	
...
static int  gfx1xm_remove(struct spi_device *spi)
{
	...
	wake_lock_destroy(&fp_wakelock);//Jorbo 20171016
    FUNC_ENTRY();
    ...
    device_destroy(gfx1xm_dev->gfx1xm_spi_class, gfx1xm_dev->devt);
    clear_bit(MINOR(gfx1xm_dev->devt), minors);
	kthread_stop(gfx1xm_dev->gfx1xm_irq_thread);
